...
DXMatrix33& DXMatrix33::SetRotation(const DXQuaternion& _quat)
{
	...
	xs = _quat.m_x+_quat.m_x;   
	ys = _quat.m_y+_quat.m_y;
	zs = _quat.m_z+_quat.m_z;
	wx = _quat.m_w*xs;
	wy = _quat.m_w*ys;
	wz = _quat.m_w*zs;
	xx = _quat.m_x*xs;
...
{
	m_v[0] = _x; m_v[5] = _y; m_v[10] = _z; m_v[15] = _w;
	return *this;
}
...
void DXMatrix44::SetColumn(const float _x,const float _y,const float _z,
						   const float _w, unsigned int _column)
{
	if(_column > 3)
		return;
	switch(_column)
...
void DXMatrix44::SetRow(const float _x, const float _y, const float _z, 
						const float _w, unsigned int _row)
{
	if(_row > 3)
		return;
	switch(_row)
...
void DXMatrix44::SetRotateX(const float &_angle)
{
	float sine,cos;
	...
	m_v[0] = m_v[1] = m_v[2] = m_v[3] = m_v[4] = m_v[7] = m_v[8] = 0.0f;
	m_v[11]= m_v[12] = m_v[13] = m_v[14] = m_v[15] =  1.0f;
}

void DXMatrix44::SetRotateY(const float &_angle)
{
	float sine,cos;
	DXMath::SinCos(_angle,sine,cos);
	m_v[0] = m_v[10] = cos;
...
void DXMatrix44::SetRotateZ(const float &_angle)
{
	float sine,cos;
	DXMath::SinCos(_angle,sine,cos);
	m_v[0] = m_v[5] = cos;
...
DXMatrix44 DXMatrix44::operator*(const DXMatrix44& _mtx44)const
{
	DXMatrix44 result;
	//column 1
	result.m_v[0] = m_v[0]*_mtx44.m_v[0] + m_v[4]*_mtx44.m_v[1] + m_v[8]*_mtx44.m_v[2] 
	+ m_v[12]*_mtx44.m_v[3];
	result.m_v[1] = m_v[1]*_mtx44.m_v[0] + m_v[5]*_mtx44.m_v[1] + m_v[9]*_mtx44.m_v[2] 
	+ m_v[13]*_mtx44.m_v[3];
	result.m_v[2] = m_v[2]*_mtx44.m_v[0] + m_v[6]*_mtx44.m_v[1] + m_v[10]*_mtx44.m_v[2] 
	+ m_v[14]*_mtx44.m_v[3];
	result.m_v[3] = m_v[3]*_mtx44.m_v[0] + m_v[7]*_mtx44.m_v[1] + m_v[11]*_mtx44.m_v[2] 
	+ m_v[15]*_mtx44.m_v[3];

	result.m_v[4] = m_v[0]*_mtx44.m_v[4] + m_v[4]*_mtx44.m_v[5] + m_v[8]*_mtx44.m_v[6] 
	+ m_v[12]*_mtx44.m_v[7];
	result.m_v[5] = m_v[1]*_mtx44.m_v[4] + m_v[5]*_mtx44.m_v[5] + m_v[9]*_mtx44.m_v[6] 
	+ m_v[13]*_mtx44.m_v[7];
	result.m_v[6] = m_v[2]*_mtx44.m_v[4] + m_v[6]*_mtx44.m_v[5] + m_v[10]*_mtx44.m_v[6] 
	+ m_v[14]*_mtx44.m_v[7];
	result.m_v[7] = m_v[3]*_mtx44.m_v[4] + m_v[7]*_mtx44.m_v[5] + m_v[11]*_mtx44.m_v[6] 
	+ m_v[15]*_mtx44.m_v[7];

	result.m_v[8] = m_v[0]*_mtx44.m_v[8] + m_v[4]*_mtx44.m_v[9] + m_v[8]*_mtx44.m_v[10] 
	+ m_v[12]*_mtx44.m_v[11];
	result.m_v[9] = m_v[1]*_mtx44.m_v[8] + m_v[5]*_mtx44.m_v[9] + m_v[9]*_mtx44.m_v[10] 
	+ m_v[13]*_mtx44.m_v[11];
	result.m_v[10] = m_v[2]*_mtx44.m_v[8] + m_v[6]*_mtx44.m_v[9] + m_v[10]*_mtx44.m_v[10] 
	+ m_v[14]*_mtx44.m_v[11];
	result.m_v[11] = m_v[3]*_mtx44.m_v[8] + m_v[7]*_mtx44.m_v[9] + m_v[11]*_mtx44.m_v[10] 
	+ m_v[15]*_mtx44.m_v[11];

	result.m_v[12] = m_v[0]*_mtx44.m_v[12] + m_v[4]*_mtx44.m_v[13] + m_v[8]*_mtx44.m_v[14] 
	+ m_v[12]*_mtx44.m_v[15];
	result.m_v[13] = m_v[1]*_mtx44.m_v[12] + m_v[5]*_mtx44.m_v[13] + m_v[9]*_mtx44.m_v[14] 
	+ m_v[13]*_mtx44.m_v[15];
	result.m_v[14] = m_v[2]*_mtx44.m_v[12] + m_v[6]*_mtx44.m_v[13] + m_v[10]*_mtx44.m_v[14] 
	+ m_v[14]*_mtx44.m_v[15];
	result.m_v[15] = m_v[3]*_mtx44.m_v[12] + m_v[7]*_mtx44.m_v[13] + m_v[11]*_mtx44.m_v[14] 
	+ m_v[15]*_mtx44.m_v[15];
	return result;
}
...
DXMatrix44& DXMatrix44::SetRotation(const DXQuaternion& _quat)
{
	float xs, ys, zs,wx,wy,wz,xx,xy,xz,yy,yz,zz;
	xs = _quat.m_x+_quat.m_x;   
	ys = _quat.m_y+_quat.m_y;
...
void DXMatrix44AffineTransform(const DXVector3& _scale, DXQuaternion _rotate, const DXVector3& _trans, DXMatrix44& _mtxOut)
{
	_mtxOut.Clear();
	_mtxOut.SetDiagonal(_scale.m_x, _scale.m_y, _scale.m_z);
	DXMatrix44 rotateMtx;
	if(!_rotate.IsUnit())
		_rotate.Normalize();
	rotateMtx.ToIdentity();
	rotateMtx.SetRotation(_rotate);
	_mtxOut =  _mtxOut * rotateMtx;
	_mtxOut.SetTranslation(_trans);
}