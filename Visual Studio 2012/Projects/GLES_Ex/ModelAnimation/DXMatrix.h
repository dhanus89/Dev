...
public:
	...
	///-------------------------------------------------------------------------------------------------
	/// <summary> set the matrix to rotation matrix with given quaternion</summary>
	///
	/// <param name=" _quat"> the rotation quaternion (must be unit length) </param>
	///
	/// <returns> the mtrix itself </returns>
	///-------------------------------------------------------------------------------------------------
	DXMatrix44& SetRotation(const DXQuaternion& _quat);
	
	void SetColumns(const DXVector4& _column1,const DXVector4& _column2,
				const DXVector4& _column3,const DXVector4& _column4);

	DXVector4 GetRow(unsigned int _row)const;
	void SetRotateX(const float &_angle);
	void SetRotateY(const float &_angle);
	void SetRotateZ(const float &_angle);
	...
	friend void DXMatrix44AffineTransform(const DXVector3& _scale,DXQuaternion _rotate, 
										const DXVector3& _trans, DXMatrix44& _mtxOut);
